...
class EddyQuadInputSpec(FSLCommandInputSpec):
    ...
    idx_file = File(
        ...
        argstr="--eddyIdx %s",
        desc=(
            "File containing indices for all volumes into acquisition "
            "parameters"
        ),
    )
    param_file = File(
...
class EddyQuadOutputSpec(TraitedSpec):
    qc_json = File(
        exists=True,
        desc=(
            "Single subject database containing quality metrics and data "
            "info."
        ),
    )
    ...
    def _list_outputs(self):
        ...
        if not isdefined(self.inputs.output_dir):
            out_dir = os.path.abspath(
                os.path.basename(self.inputs.base_name) + ".qc"
            )
        else:
        ...
        outputs["avg_b_png"] = sorted(
            glob(os.path.join(out_dir, "avg_b*.png"))
        )
