...
the next step is executing the ``dmriprep`` command-line.
The ``dmriprep`` command-line options are documented in the :ref:`usage`
section.
The ``dmriprep`` command-line adheres to the `BIDS-Apps recommendations
...
container execution followed by the ``dmriprep`` command-line options
...
under the ``nipreps/dmriprep`` identifier.
...
Please make sure you `follow our tips and tricks to run dMRIPrep's Singularity images
<singularity.html>`_.
...
Make sure all of *dMRIPrep*'s `External Dependencies`_ are installed.
