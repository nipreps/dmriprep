...
def init_dwi_preproc_wf(dwi_file):
    ...
    from nipype.interfaces import (freesurfer as fs, fsl, mrtrix3, ants, \
                                   io as nio, utility as niu)
    ...
    denoise = pe.Node(mrtrix3.DWIDenoise(noise='noise.nii.gz',
                                         out_file='denoised.nii.gz'),
                      name="denoise")

    # name unring using fname_presuffix
    unring = pe.Node(mrtrix3.MRDeGibbs(out_file='unringed.nii.gz'),
                     name="unring")
    ...
    def gen_index(in_file):
        ...
        out_file = fname_presuffix(in_file, suffix="_index.txt", newpath=op.abspath('.'), use_ext=False)
    ...
    def gen_acqparams(in_file, metadata):
        ...
        out_file = fname_presuffix(in_file, suffix="_acqparams.txt", newpath=op.abspath('.'), use_ext=False)
    ...
    def b0_average(in_dwi, in_bval, b0_thresh=10.0, out_file=None):
        ...
        if out_file is None:
            out_file = fname_presuffix(in_dwi, suffix="_avg_b0", newpath=op.abspath('.'))
    ...
    ecc = pe.Node(fsl.Eddy(repol=True,
                           cnr_maps=True,
                           residuals=True,
                           method='jac'),
                 name='fsl_eddy')
    ...
    dwi_wf.connect(
        [(inputnode, denoise, [('dwi_file', 'in_file')]),
         ...
         # does avg_b0_1 need to be run?
         (ecc, avg_b0_1, [('out_corrected', 'in_dwi')]),
         (inputnode, avg_b0_1, [('bval_file', 'in_bval')]),
