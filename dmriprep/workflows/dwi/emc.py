...
from dmriprep.interfaces.bids import DerivativesDataSink
from dmriprep.interfaces.images import SignalPrediction, CalculateCNR, ReorderOutputs, ImageMath, ExtractB0, \
    MatchTransforms, RescaleB0
from dmriprep.interfaces.reports import IterationSummary, HMCReport
from dmriprep.interfaces.vectors import ReorientVectors, CheckGradientTable, CombineMotions
from dmriprep.utils.images import save_4d_to_3d, save_3d_to_4d, prune_b0s_from_dwis
from dmriprep.utils.vectors import _rasb_to_bvec_list, _rasb_to_bval_floats


def linear_alignment_workflow(transform="Rigid", iternum=0, precision="precise"):
    ...
    linear_alignment_inputnode = pe.Node(
        niu.IdentityInterface(fields=input_node_fields), name='linear_alignment_inputnode')
    ...
    linear_alignment_outputnode = pe.Node(
        niu.IdentityInterface(fields=["registered_image_paths", "affine_transforms",
                                      "updated_template"]), name='linear_alignment_outputnode')
    ants_settings = pkgrf(
        "dmriprep",
        "config/emc_{precision}_{transform}.json".format(precision=precision, transform=transform))
    ...
    iteration_wf.connect(linear_alignment_inputnode, "image_paths", iter_reg, "moving_image")
    iteration_wf.connect(linear_alignment_inputnode, "template_image", iter_reg, "fixed_image")
    ...
    iteration_wf.connect(invert_average, "output_image", linear_alignment_outputnode,
                         "updated_template")
    iteration_wf.connect(iter_reg, "forward_transforms", linear_alignment_outputnode,
                         "affine_transforms")
    iteration_wf.connect(iter_reg, "warped_image", linear_alignment_outputnode,
                         "registered_image_paths")

    return iteration_wf


def init_b0_emc_wf(transform="Rigid", metric="Mattes", num_iters=3, name="b0_emc_wf"):
    ...
    initial_reg = linear_alignment_workflow(
        ...
        precision="coarse",
        iternum=0)
    b0_emc_wf.connect(b0_emc_inputnode, "initial_template", initial_reg, "linear_alignment_inputnode.template_image")
    b0_emc_wf.connect(b0_emc_inputnode, "b0_images", initial_reg, "linear_alignment_inputnode.image_paths")
    ...
    for iternum in range(1, num_iters):
        reg_iters.append(
            linear_alignment_workflow(
                ...
                precision="precise",
                iternum=iternum))
        b0_emc_wf.connect(reg_iters[-2], "linear_alignment_outputnode.updated_template", reg_iters[-1],
                          "linear_alignment_inputnode.template_image")
        b0_emc_wf.connect(b0_emc_inputnode, "b0_images", reg_iters[-1], "linear_alignment_inputnode.image_paths")
        b0_emc_wf.connect(reg_iters[-1],
                          "linear_alignment_outputnode.updated_template", iter_templates, "in%d" % (iternum + 1))

    # Attach to outputs
    # The last iteration aligned to the output from the second-to-last
    b0_emc_wf.connect(reg_iters[-2], "linear_alignment_outputnode.updated_template", b0_emc_outputnode,
                      "final_template")
    b0_emc_wf.connect(reg_iters[-1], "linear_alignment_outputnode.affine_transforms", b0_emc_outputnode,
                      "forward_transforms")
    b0_emc_wf.connect(reg_iters[-1], "linear_alignment_outputnode.registered_image_paths", b0_emc_outputnode,
                      "aligned_images")
...
def init_enhance_and_skullstrip_template_mask_wf(name):
    ...
    eastm_inputnode = pe.Node(niu.IdentityInterface(fields=['in_file']),
                        name='eastm_inputnode')
    eastm_outputnode = pe.Node(niu.IdentityInterface(fields=[
        'mask_file', 'skull_stripped_file', 'bias_corrected_file']), name='eastm_outputnode')
    ...
    eastm_workflow.connect([
        ...
        (apply_mask, eastm_outputnode, [('output_product_image', 'skull_stripped_file')]),
...
def init_emc_model_iteration_wf(transform, precision="coarse", name="emc_model_iter0"):
    emc_model_iter_workflow = pe.Workflow(name=name)
    emc_model_iteration_inputnode = pe.Node(niu.IdentityInterface(fields=['original_dwi_files', 'original_rasb_file',
                                                                          'aligned_dwi_files', 'aligned_vectors',
                                                                          'b0_median', 'b0_mask', 'b0_indices']),
                                            name='emc_model_iteration_inputnode')

    emc_model_iteration_outputnode = pe.Node(
        niu.IdentityInterface(
            fields=['emc_transforms', 'aligned_dwis', 'aligned_vectors',
                    'predicted_dwis', 'motion_params']),
        name='emc_model_iteration_outputnode')

    ants_settings = pkgrf(
        "dmriprep",
        "config/emc_{precision}_{transform}.json".format(precision=precision,
                                                         transform=transform))
    ...
    emc_model_iter_workflow.connect([
        ...
        (emc_model_iteration_inputnode, predict_dwis, [('aligned_dwi_files', 'aligned_dwi_files'),
        ...
        (emc_model_iteration_inputnode, register_to_predicted, [
            ('original_dwi_files', 'moving_image'),
            ('b0_mask', 'fixed_image_mask')]),
        (register_to_predicted, calculate_motion, [
            (('forward_transforms', _list_squeeze), 'transform_files')]),
        (emc_model_iteration_inputnode, calculate_motion, [('original_dwi_files', 'source_files'),
                                                           ('b0_median', 'ref_file')]),
        (calculate_motion, emc_model_iteration_outputnode, [('motion_file', 'motion_params')]),
        (register_to_predicted, post_vector_transforms, [
            (('forward_transforms', _list_squeeze), 'affines')]),
        (emc_model_iteration_inputnode, post_vector_transforms, [('original_rasb_file', 'rasb_file')]),
        (predict_dwis, emc_model_iteration_outputnode, [('predicted_image', 'predicted_dwis')]),
        (post_vector_transforms, emc_model_iteration_outputnode, [('out_rasb', 'aligned_vectors')]),
        (register_to_predicted, emc_model_iteration_outputnode, [('warped_image', 'aligned_dwis'),
...
def init_dwi_model_emc_wf(transform, num_iters=2, name='dwi_model_emc_wf'):
    dwi_model_emc_wf_workflow = pe.Workflow(name=name)
    inputnode = pe.Node(
        niu.IdentityInterface(
            fields=['original_dwi_files', 'original_rasb_file', 'aligned_dwi_files', 'b0_indices',
                    'initial_transforms', 'aligned_vectors', 'b0_median', 'b0_mask', 'warped_b0_images']),
        name='dwi_model_emc_inputnode')

    outputnode = pe.Node(
        niu.IdentityInterface(
            fields=['emc_transforms', 'model_predicted_images', 'cnr_image',
                    'optimization_data']),
    ...
    initial_model_iteration = init_emc_model_iteration_wf(
        transform, precision="coarse", name="initial_model_iteration")
    ...
    dwi_model_emc_wf_workflow.connect([
        ...
        (inputnode, initial_model_iteration, [
            ...
            ('aligned_vectors', 'emc_model_iteration_inputnode.aligned_vectors'),
            ('aligned_dwi_files', 'emc_model_iteration_inputnode.aligned_dwi_files')]),
    ...
    for iteration_num in range(num_iters - 1):
        iteration_name = 'HMC_iteration%03d' % (iteration_num + 1)
        motion_key = 'in%d' % (iteration_num + 2)
        model_iterations.append(
            init_emc_model_iteration_wf(transform=transform,
                                        precision="precise",
                                        name=iteration_name)
        )
        dwi_model_emc_wf_workflow.connect([
            ...
            (inputnode, model_iterations[-1], [
                ...
                ('b0_indices', 'emc_model_iteration_inputnode.b0_indices'),
                ('original_dwi_files', 'emc_model_iteration_inputnode.original_dwi_files'),
                ('original_rasb_file', 'emc_model_iteration_inputnode.original_rasb_file'),
                ('b0_median', 'emc_model_iteration_inputnode.b0_median')]),
    ...
    # Create a report:
    emc_report = pe.Node(HMCReport(), name='emc_report')
    ds_report_emc_gif = pe.Node(
        DerivativesDataSink(suffix="emc_animation"), name='ds_report_emc_gif',
    ...
    #         DerivativesDataSink(suffix="emc_iterdata"), name='ds_report_iteration_plot',
    ...
    #         (summarize_iterations, emc_report, [
    #             ('iteration_summary_file', 'iteration_summary')])
    #     ])

    dwi_model_emc_wf_workflow.connect([
        (model_iterations[-1], reorder_dwi_xforms, [
            ('emc_model_iteration_outputnode.emc_transforms', 'model_based_transforms'),
            ('emc_model_iteration_outputnode.predicted_dwis', 'model_predicted_images'),
            ('emc_model_iteration_outputnode.aligned_dwis', 'warped_dwi_images')]),
        ...
        (reorder_dwi_xforms, emc_report, [
            ('full_predicted_dwi_series', 'model_predicted_images'),
            ('emc_warped_images', 'registered_images')]),
        (emc_report, ds_report_emc_gif, [('plot_file', 'in_file')])
    ])
...
def init_emc_wf(name, mem_gb=3, omp_nthreads=8):
    import_list = ["import warnings", "warnings.filterwarnings(\"ignore\")", "import sys", "import os",
                   "import numpy as np", "import networkx as nx", "import nibabel as nb",
                   "from nipype.utils.filemanip import fname_presuffix"]

    emc_wf = pe.Workflow(name=name)

    meta_inputnode = pe.Node(niu.IdentityInterface(fields=['dwi_file', 'in_bval', 'in_bvec', 'b0_template']),
                             name='meta_inputnode')
    ...
    split_b0s_node = pe.Node(niu.Function(input_names=['in_file'], output_names=['out_files'],
                                          function=save_4d_to_3d, imports=import_list),
                             name="split_b0s_node")

    prune_b0s_from_dwis_node = pe.Node(niu.Function(input_names=['in_files', 'b0_ixs'], output_names=['out_files'],
                                          function=prune_b0s_from_dwis, imports=import_list),
                                       name="prune_b0s_from_dwis_node")

    split_dwis_node = pe.Node(niu.Function(input_names=['in_file'], output_names=['out_files'],
                                           function=save_4d_to_3d, imports=import_list),
                              name="split_dwis_node")

    merge_b0s_node = pe.Node(niu.Function(input_names=['in_files'], output_names=['out_file'],
                                          function=save_3d_to_4d, imports=import_list),
                             name="merge_b0s_node")

    match_transforms_node = pe.Node(MatchTransforms(), name="match_transforms_node")

    b0_emc_wf = init_b0_emc_wf(transform="Rigid")

    eastm_wf = init_enhance_and_skullstrip_template_mask_wf(name='enhance_and_skullstrip_template_mask_wf')
    ...
    # Rotate vectors
    b0_based_vector_transforms = pe.Node(ReorientVectors(), name="b0_based_vector_transforms")

    # Grab the median of the aligned B0 images
    b0_median = pe.Node(RescaleB0(), name='b0_median')

    # Do model-based motion correction
    dwi_model_emc_wf = init_dwi_model_emc_wf(transform='Affine', num_iters=2)
    ...
    emc_wf.connect([
        ...
        (meta_inputnode, vectors_node, [('dwi_file', 'dwi_file'),
                                        ('in_bval', 'in_bval'),
                                        ('in_bvec', 'in_bvec')]),
        ...
        (meta_inputnode, b0_emc_wf, [('b0_template', 'b0_emc_inputnode.initial_template')]),
        (b0_emc_wf, match_transforms_node, [(('b0_emc_outputnode.forward_transforms', _list_squeeze), 'transforms')]),
        ...
        (match_transforms_node, b0_based_vector_transforms, [('transforms', 'affines')]),
        (vectors_node, b0_based_vector_transforms, [('out_rasb', 'rasb_file')]),
        (split_dwis_node, b0_based_image_transforms, [('out_files', 'input_image')]),
        (match_transforms_node, b0_based_image_transforms, [('transforms', 'transforms')]),
        (b0_emc_wf, merge_b0s_node, [('b0_emc_outputnode.aligned_images', 'in_files')]),
        (merge_b0s_node, b0_median, [('out_file', 'in_file')]),
        (eastm_wf, b0_median, [('eastm_outputnode.mask_file', 'mask_file')]),
        (b0_median, b0_based_image_transforms, [('out_ref', 'reference_image')]),
        ...
        (prune_b0s_from_dwis_node, dwi_model_emc_wf, [('out_files', 'dwi_model_emc_inputnode.original_dwi_files')]),
        (vectors_node, dwi_model_emc_wf, [('out_rasb', 'dwi_model_emc_inputnode.original_rasb_file')]),
        (b0_based_image_transforms, dwi_model_emc_wf, [('output_image', 'dwi_model_emc_inputnode.aligned_dwi_files')]),
        (b0_based_vector_transforms, dwi_model_emc_wf, [('out_rasb', 'dwi_model_emc_inputnode.aligned_vectors')]),
        (b0_median, dwi_model_emc_wf, [('out_ref', 'dwi_model_emc_inputnode.b0_median')]),
        (eastm_wf, dwi_model_emc_wf,
         [('eastm_outputnode.mask_file', 'dwi_model_emc_inputnode.b0_mask')]),
        (vectors_node, dwi_model_emc_wf, [('b0_ixs', 'dwi_model_emc_inputnode.b0_indices')]),
        (match_transforms_node, dwi_model_emc_wf, [('transforms', 'dwi_model_emc_inputnode.initial_transforms')]),
        (b0_emc_wf, dwi_model_emc_wf,
         [('b0_emc_outputnode.aligned_images', 'dwi_model_emc_inputnode.warped_b0_images')]),
        (b0_emc_wf, meta_outputnode, [('b0_emc_outputnode.final_template', 'final_template')]),
        (dwi_model_emc_wf, meta_outputnode, [('dwi_model_emc_outputnode.emc_transforms', 'forward_transforms'),
                                             ('dwi_model_emc_outputnode.optimization_data', 'optimization_data'),
                                             ('dwi_model_emc_outputnode.cnr_image', 'cnr_image')]),
        (b0_emc_wf, uncorrect_model_images, [('b0_emc_outputnode.final_template', 'reference_image')]),
        (dwi_model_emc_wf, uncorrect_model_images, [('dwi_model_emc_outputnode.emc_transforms', 'transforms')]),
        (split_dwis_node, uncorrect_model_images, [('out_files', 'input_image')]),
        (uncorrect_model_images, meta_outputnode, [('output_image', 'noise_free_dwis')])
    ])
    return emc_wf
